// Shared, parameterized eval scorers.
//
// These are the reusable scoring internals extracted from the live eval harness so a second
// harness (the Meridian stress harness) can reuse them against a different bundle/labels/oracle.
// All scoring logic is verbatim from the original harness — only the three Helios-specific bits
// (bundle dir, labels path, the .txt/mime filter, and the hardcoded LINK_CASES) are parameterized
// via EvalConfig. No DB/LLM access changes — the scorers still read the live graph.
...
export type TypeName = keyof typeof TABLES_BY_TYPE;
...
// A labeled mention that exercises lexical + semantic linking (a paraphrase grounded in the bundle
// text), expecting a specific canonical entity as the top hit.
export type LinkCase = { mention: string; type: TypeName; expectedLabel: string };

// Per-bundle configuration for the reusable scorers/ingest. `bundleDir` is read for ingest;
// `accept` filters filenames; `mime` is passed to extractText; `linkCases` drives scoreLinking.
...
// Live-graph helpers
// ---------------------------------------------------------------------------
export async function liveEntities(type: TypeName): Promise<Array<Record<string, any>>> {
...
export async function aliasMap(type: TypeName): Promise<Map<number, string[]>> {
...
export async function idByLabel(type: TypeName, label: string): Promise<number | null> {
...
// 1. Classification
// ---------------------------------------------------------------------------
export async function scoreClassification(labels: Labels): Promise<Scorecard["classification"]> {
...
export async function scoreExtraction(labels: Labels): Promise<Scorecard["extraction"]> {
...
// ---------------------------------------------------------------------------
export async function scoreResolution(labels: Labels): Promise<Scorecard["resolution"]> {
...
// ---------------------------------------------------------------------------
export async function scoreLinking(linkCases: LinkCase[]): Promise<Scorecard["linking"]> {
...
export async function scoreLinks(labels: Labels): Promise<Scorecard["links"]> {
...
export async function resolveCQParams(template: string, params: Record<string, unknown>): Promise<Record<string, number> | null> {
...
export async function labelsOf(type: TypeName, ids: number[]): Promise<string[]> {
  const live = await liveEntities(type);
...
export async function compareCQ(
  id: string,
  template: string,
  rows: any[],
  gold: any,
...
export async function scoreCQ(labels: Labels): Promise<Scorecard["cq"]> {
...
// Read cfg.bundleDir, filter filenames by cfg.accept, extract via cfg.mime, insert each doc, run
// the advance() loop to a terminal status. Returns filename -> {id, status}.
export async function ingestBundle(cfg: EvalConfig): Promise<Map<string, { id: number; status: string }>> {
  const out = new Map<string, { id: number; status: string }>();
  const files = fs.readdirSync(cfg.bundleDir).filter(cfg.accept).sort();
  log(`[eval] ingesting ${files.length} fixture files live (classify -> index -> extract -> resolve -> link)...`);
  for (const file of files) {
    const buf = fs.readFileSync(path.join(cfg.bundleDir, file));
    const { rawText } = await extractText(buf, cfg.mime);
    const [doc] = await db
      .insert(documents)
      .values({ filename: file, mimeType: cfg.mime, rawText, status: "ingested" })
      .returning();
    ...
    out.set(file, { id: doc.id, status: cur });
  }
  return out;
}

// ---------------------------------------------------------------------------
// Assemble the full scorecard from the six scorers.
// ---------------------------------------------------------------------------
export async function scoreAll(cfg: EvalConfig, labels: Labels): Promise<Scorecard> {
  ...
  const linking = await scoreLinking(cfg.linkCases);
  const links = await scoreLinks(labels);
  const cq = await scoreCQ(labels);
  return { classification, extraction, resolution, linking, links, cq };
}
